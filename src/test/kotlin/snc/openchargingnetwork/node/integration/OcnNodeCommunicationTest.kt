...
class OcnNodeCommunicationTest : BaseIntegrationTest() {
  ...
  fun `should complete full OCN node communication flow`() {
    ...
    val node1HealthAfter = restTemplate.getForEntity("$node1Url/ocn/health", String::class.java)
    val node2HealthAfter = restTemplate.getForEntity("$node2Url/ocn/health", String::class.java)
